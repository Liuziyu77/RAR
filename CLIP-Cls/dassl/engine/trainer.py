...
class TrainerX_topk(TrainerX):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output = self.model_inference(input)
            values, indices = output.topk(top_k, 1)

            for impath_, label_, value, index in zip(impath, label.data.cpu(), values.data.cpu(), indices.data.cpu()):
                results.append({impath_: {'label': label_, 'pred_logits': value, 'pred_class': index}})

        save_file_name = osp.join(self.cfg.OUTPUT_DIR, 'predictions.pth')
